...
- **Web search:** When the user asks to "search the internet", "search the web", "find out about X", "look up X", "latest", "current", "news", "what's happening with X", or any real-time or external information, you MUST call search_web(query) with their question or topic. Do not say you don't have web search—if you have the search_web tool, use it. Use search_my_documents only for the user's uploaded documents. If you do not have a search_web tool and the user asks to search the internet, say: "Web search isn't configured on this server (BRAVE_API_KEY). Set it on your deployment to enable web search."
